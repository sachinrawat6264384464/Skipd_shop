...
@router.post("/admin/bulk-create")
async def bulk_create_admin_products(
    payload: dict = Body(...),
    db: AsyncSession = Depends(get_db)
):
    """Admin: Bulk insert products into PostgreSQL DB with all 32 Enterprise columns."""
    try:
        products_data = payload.get("products", [])
        if not isinstance(products_data, list) or len(products_data) == 0:
            return {"message": "No products provided", "count": 0}

        created_count = 0
        for pdata in products_data:
            title = pdata.get("title") or "Unnamed Product"
            handle = pdata.get("handle") or title.lower().replace(" ", "-")
            sku = pdata.get("sku") or f"SKU-{int(datetime.datetime.utcnow().timestamp() * 1000)}"

            # Ensure handle & sku uniqueness
            handle_check = await db.execute(select(Product).where(Product.handle == handle))
            if handle_check.scalars().first():
                handle = f"{handle}-{int(datetime.datetime.utcnow().timestamp())}"

            # Category resolution
            category_id = None
            cat_slug = pdata.get("category_slug") or pdata.get("category")
            if cat_slug:
                cat_slug_clean = str(cat_slug).lower().replace(" ", "-")
                c_res = await db.execute(select(Category).where(Category.slug == cat_slug_clean))
                cat_obj = c_res.scalars().first()
                if not cat_obj:
                    cat_obj = Category(name=str(cat_slug).capitalize(), slug=cat_slug_clean)
                    db.add(cat_obj)
                    await db.commit()
            ...
            prod = Product(
                ...
                box_contents=pdata.get("box_contents"),
                price=float(pdata.get("price") or 0.0),
                compare_at_price=float(pdata.get("compare_at_price")) if pdata.get("compare_at_price") else None,
                cost_price=float(pdata.get("cost_price")) if pdata.get("cost_price") else None,
                sku=sku,
                barcode=pdata.get("barcode"),
                stock_quantity=int(pdata.get("stock_quantity") or 0),
                low_stock_threshold=int(pdata.get("low_stock_threshold") or 10),
                category_id=category_id,
                sub_category=pdata.get("sub_category"),
                brand=pdata.get("brand"),
                warehouse=pdata.get("warehouse"),
                image_url=pdata.get("image_url") or (pdata.get("images")[0] if pdata.get("images") else None),
                images=pdata.get("images") or [],
                video_url=pdata.get("video_url"),
                color=pdata.get("color"),
                size=pdata.get("size"),
                material=pdata.get("material"),
                weight=float(pdata.get("weight")) if pdata.get("weight") else None,
                dimensions=pdata.get("dimensions"),
                gst_rate=float(pdata.get("gst_rate")) if pdata.get("gst_rate") else None,
                hsn_code=pdata.get("hsn_code"),
                country_of_origin=pdata.get("country_of_origin"),
                tags=pdata.get("tags") or [],
                meta_title=pdata.get("meta_title"),
                meta_description=pdata.get("meta_description"),
                is_featured=bool(pdata.get("is_featured") or False),
                is_active=bool(pdata.get("is_active") if "is_active" in pdata else True)
            )
            db.add(prod)
            created_count += 1
        ...
        return {"message": f"Successfully created {created_count} products", "count": created_count}
    except Exception as err:
        await db.rollback()
        print(f"[BULK CREATE ERROR] {err}")
        raise HTTPException(status_code=500, detail=f"Failed to bulk create products: {str(err)}")
