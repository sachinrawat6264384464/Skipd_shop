    ...
    try:
        ...
        for idx, item in enumerate(products_list):
            ...
            prod = Product(
                ...
                warehouse=item.get("warehouse") or "Main FC",
                images=images_list,
                gallery_images=item.get("gallery_images") if isinstance(item.get("gallery_images"), list) else [],
                ...
                gst_rate=float(item.get("gst_rate")) if item.get("gst_rate") else 18.0,
                hsn_code=item.get("hsn_code"),
                country_of_origin=item.get("country_of_origin") or "India",
                tags=tags_list,
                highlights=item.get("highlights") if isinstance(item.get("highlights"), list) else [],
                box_contents=item.get("box_contents") if isinstance(item.get("box_contents"), list) else [],
                meta_title=item.get("meta_title"),
                meta_description=item.get("meta_description"),
                is_featured=bool(item.get("featured", True)),
